...
import { getNextPlayer } from '../constants';
    ...
    <section className={tm('flex flex-col space-y-2 items-center', 'w-40')}>
      ...
      <ol className={tm('flex flex-col space-y-1')}>
        {historyCount.map((_, index) => {
          ...
          const playerIcon = getNextPlayer(index-1); // 현재 차례의 플레이어 아이콘 가져오기
          const content = `${playerIcon} 게임 #${gameIndex < 10 ? `0${gameIndex}` : gameIndex}`;
